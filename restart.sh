...
ID=$(sed -n '1p' "$KEY_FILE" | tr -d '[:space:]')    # 讀取第一行作為 ID
GITHUB_TOKEN=$(sed -n '2p' "$KEY_FILE" | tr -d '[:space:]')  # 讀取第二行作為 Token

if [[ -z "$ID" || -z "$GITHUB_TOKEN" ]]; then
    echo "錯誤: key.txt 內容格式錯誤，請確保第一行是 ID，第二行是 GitHub Token"
    exit 1
fi
...
process_container() {
    ...
    if [[ "$update_flag" == "Y" ]]; then
        ...
        echo "[$container_name] 下載 GitHub 腳本..."
        ...
        if [[ -n "$accounts_file" ]]; then
            echo "[$container_name] 下載 GitHub 帳號設定 ($accounts_file)..."
            curl -s -H "Accept: application/vnd.github.v3.raw" -H "Authorization: token ${GITHUB_TOKEN}" -o "${BASE_DIR}/$project_name/$accounts_file" "$GITHUB_API/${project_name}/${ID}/$accounts_file"
        fi
